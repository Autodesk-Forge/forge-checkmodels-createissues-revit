...
// Note: this is for local testing. Comment this before you run the cloud version.
...
namespace FindColumnsIO
{
...
#endif
#if (Cloud)
    public class FindColumnsIOApp : IExternalDBApplication
#endif
#if (LocalTest)
    public class FindColumnsIOApp : IExternalDBApplication
#endif 

    {
        ...
        //=======================================================
        //  FindColumnsIO main function 
        //=======================================================
        private void FindColumnsIOMain(Document doc)
        {
            // Initialization 
            ...
            if (m_allColumnsOnWalls.Count > 0)
            {
                foreach (ElementId id in m_allColumnsOnWalls)
                {
                    ...
                    toSelected.Add(familyInstance.Id);
                    // collect to write to file
                    //elemList += familyInstanceId.ToString() + ", "; 
                    elemList += familyInstance.UniqueId.ToString() + ", ";
                }

                // take out extra two chars at the end. (there may be a better way.) 
                if(elemList.Length>0)
                {
                    elemList = elemList.TrimEnd(' ');
                    elemList = elemList.TrimEnd(','); 
                }
                // MH: This is the UI part. 
                //selection.SetElementIds(toSelected);

                // Output to as a simple list of elemend it. e.g, "1234, 2334, 4336" 
                // Setup file location in temp directory
                // MH Note: working directory changes  
                // (1) in Temp folder (for desktop) 
#if (Desktop)
                string folder = Environment.GetEnvironmentVariable("TEMP");
                string outputFile = System.IO.Path.Combine(folder, "result.txt");
#endif
                // (2) in my local tmp
                //StreamWriter writer = new StreamWriter("C:\\tmp\\result.txt");
                // (3) working folder (This goes to Revit install on desktop, which I do not have permission) 
#if (Cloud)
                string outputFile = "result.txt";
#endif
                StreamWriter writer = new StreamWriter(outputFile); 
                writer.WriteLine(elemList);
                writer.Close(); 
            }
            //return Result.Succeeded;
        }
...
#region Class Implementation
        ...
        //private double GetElevationForRay(Wall wall, Document doc)
        {
        ...
        private void Get3DView(string viewName, Document doc)
        {
